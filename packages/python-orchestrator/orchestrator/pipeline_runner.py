...
class Pipeline:
    def __init__(self, repo_path: str, output_path: str, daemon_path: str):
        ...
        self.name_registry = {}
        self.import_graph = {}
        self.db_conn = sqlite3.connect(db_path) # Connect to the now-guaranteed-fresh path
        self._init_db()
                        ...
                        if self.repo_path in resolved_path.parents:
                            ...
                            for ext in ['.ts', '.js', '.py', '.tsx', '.jsx']:
                                if (resolved_path.parent / f"{resolved_path.name}{ext}").exists():
                                    resolved_imports.append(str((resolved_path.parent / f"{resolved_path.name}{ext}").relative_to(self.repo_path)))
                                    found = True
                                    break
                            if not found and (resolved_path / "__init__.py").exists():
                                resolved_imports.append(str((resolved_path / "__init__.py").relative_to(self.repo_path)))
    ...
    def generate_deterministic_brief(self, top_n_modules=10, complexity_threshold=10):
        ...
        filtered_graph = {
            node: deps for node, deps in self.import_graph.items() 
        ...
        sorted_modules = sorted(inbound_counts.items(), key=lambda item: item[1], reverse=True)
        key_modules = sorted_modules[:top_n_modules]
        # --- END FIX 2 ---

        brief.append("## Key Architectural Modules\n")
        if key_modules:
            brief.append("Based on import centrality, the most important modules are:\n")
        ...
        if hotspots:
            ...
            brief.append(f"Functions with a cyclomatic complexity of {complexity_threshold} or higher:\n")
            for file_path, symbol_name, complexity in hotspots:
                brief.append(f"- `{file_path}:{symbol_name}` (Complexity: {complexity})")
            brief.append("\n---\n")
        ...
        for path, _ in key_modules:
            ...
            for fqn, data in self.name_registry.items():
                if data['file_path'] == path and data.get('is_public', False):
                    try:
                        ...
                        if first_line: public_apis.append(f"`{first_line}`")
                    except Exception:
                        continue
            ...
            brief.append(f"**Dependencies:** Imports `{len(self.import_graph.get(path, []))}` other local modules.\n")
